...
- Pi keeps a host-independent `PlanningSession` per logical request with states `collecting-direction`, `collecting-name`, `awaiting-create`, `cancelled`, `create-failed`, `identity-unknown`, `task-created`, and `planning`. Questions collect a bounded title and goal/scope; the workflow tool owns the single native confirmation, refreshes the Provider snapshot after create, and returns structured workflow state. A cancelled create transitions to `cancelled`, permits recollection, and never marks the task as created. A failed create transitions to `create-failed` so the user can correct and retry without leaving the question guard in `awaiting-create`. An unresolvable create transitions to `identity-unknown` and blocks duplicate creation until inspection.
- Mutation recovery records the actual pre-mutation revision, stable target, pre-mutation target status, and pre-mutation current-task identity. `start` requires a newly active target with recorded pre-status evidence, `finish` requires a successful public current-task read showing the recorded pointer was cleared, and `archive` requires the recorded target to leave the active task snapshot; revision change alone is never success evidence. Provider snapshots are refreshed after mutation errors because Trellis can partially mutate before returning a failure.
