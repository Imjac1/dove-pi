...
- 失败的 `v0.1.3`、`v0.1.4` 标签保持不变；修复 Windows checkout 行尾导致的规格预算差异后，更新版本号为 `0.1.5`，保持 Pi、Trellis 和扩展锁定版本不变。
...
- `v0.1.3` 和 `v0.1.4` 的 CI 均失败并保持不可变；完整日志确认后者由 CRLF checkout 使规格原始字节预算越界。规范化计量修复改由 `v0.1.5` 发布，不改写远端历史。
...
- `--offline --version` 仍可能由 Pi 接管；本任务记录该兼容行为，但不把它作为 `0.1.5` 的发布阻断项，除非发布回归证明会影响普通启动。
