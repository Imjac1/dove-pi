...
import { collectCacheUsageSamples, inspectCacheDiagnostics, type CacheDiagnostics } from "../pi-adapter/cache-diagnostics.ts";
...
function decodeProjectName(dirName: string): string {
	return dirName.replace(/^-+|-$/g, "").split("--").join("\\");
}
...
export async function runCacheAudit(options: CacheAuditOptions = {}): Promise<SessionCacheRow[]> {
	...
	for (const dir of projectDirs) {
		...
		const stat = await import("node:fs/promises").then((fs) => fs.stat(projectPath).catch(() => undefined));
		if (!stat?.isDirectory()) continue;

		let files: string[] = [];
		try {
			files = (await readdir(projectPath)).filter((f) => f.endsWith(".jsonl")).map((f) => join(projectPath, f));
		} catch {
			continue;
		}

		const project = decodeProjectName(dir);
		if (options.filter && !project.toLowerCase().includes(options.filter.toLowerCase())) continue;
		...
		for (const file of files) {
			...
			const row: SessionCacheRow = {
				session: file.split(/[\\/]/).pop()?.replace(/\.jsonl$/, "").slice(0, 21) ?? "",
				project,
...
export function formatCacheAudit(rows: readonly SessionCacheRow[]): string {
	...
	lines.push("| 会话 | 项目 | 请求 | 会话命中% | 末次命中% | warmup | 全MISS | 末次miss原因 | 未缓存input |");
	lines.push("| --- | --- | --- | --- | --- | --- | --- | --- | --- |");
	for (const r of rows) {
		lines.push(
			`| ${r.session} | ${r.project} | ${r.requests} | ${r.sessionHitRate === undefined ? "n/a" : r.sessionHitRate.toFixed(1)}% | ${r.lastHitRate === undefined ? "n/a" : r.lastHitRate.toFixed(1)}% | ${r.warmups} | ${r.fullMisses} | ${r.lastMissReason ?? "n/a"} | ${r.uncachedTokens.toLocaleString()} |`,
		);
	}
	lines.push("");
	lines.push(
	...
	lines.push("miss 原因: warmup=会话首个请求(预期) · prefix-change=前缀变动(应尽量消除) · idle=空闲超时 · model-change=换模型");
	return lines.join("\n");
}