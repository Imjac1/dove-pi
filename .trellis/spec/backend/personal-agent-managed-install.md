...
- Missing schema-2 state is valid only for first install. Mutating commands fail closed on malformed or structurally invalid state. Durable state replacement keeps one valid `install.json.bak`; repair preserves a corrupt primary for diagnosis, validates backup references, then scans managed release directories before using network recovery.
...
- The managed launcher resolves and probes Python 3.10+ on every invocation, trying `python` and `py -3` identities without retaining the installation-time executable as a single point of failure. The command wrapper uses Windows PowerShell first and PowerShell Core as a fallback while preserving the child exit code.
- The piped bootstrap keeps the complete default behavior. A downloaded `install.ps1` additionally accepts `-NoPath`, `-NoFont`, `-NoExtensions`, and an HTTP/HTTPS `-Proxy`; standard proxy environment variables are used when no explicit proxy is supplied. Release requests have bounded timeout/redirect settings and pass switches through both first install and same-release repair without changing unrelated user state.
...
- A bootstrap-provided archive/checksum/tag is SHA-verified again and copied into the managed release cache before activation so `repair` can rebuild offline. Cache keys and descriptors bind tag, version, release ID, manifest digest, and checksum; offline repair requires exact identity and bounded pruning retains current/previous recovery assets. Release tag, advertised version, and archive manifest version must match.
- Confirmed uninstall removes only Dove-owned managed children and the exact Dove `bin` entry from persisted user PATH. Recursive deletion uses a boundary-validated Windows extended-length path so a real release with npm files beyond legacy `MAX_PATH` cannot leave a partial uninstall. It never removes Pi credentials/sessions/settings/extensions, project `.trellis`, development checkouts, Python, Node.js, fonts, or unrelated PATH entries. JSON mode reports `pathRemoved` without adding human output to stdout.
...
| Managed state is malformed or structurally invalid | Install/update/rollback stop without prune; repair uses a valid backup or verified release scan and preserves the corrupt file |
...
| Cached archive metadata differs from the requested release identity | Reject the cached entry, download once to a temporary file, verify, then replace it atomically |
...
- Assert corrupt state blocks every ordinary mutation without pruning, while repair can recover from a valid backup or verified directory scan.
...
- Assert a generated launcher works after the installation-time Python path moves, skips unusable/old Python commands, and preserves `.cmd` fallback exit behavior.
...
- Cover explicit/environment proxy precedence, bounded request parameters, cleanup warnings, duplicate/link-like ZIP rejection, exact offline cache identity, and bounded cache retention.
