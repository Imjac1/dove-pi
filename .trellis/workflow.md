...
Before fixing any issue, classify it against the frozen acceptance contract:
- `blocking` / `regression`: fix only when tied to an existing acceptance ID, then re-check that acceptance.
- `follow_up`: record it and keep it out of the current task's remaining work.
- `scope_change` / `serious_unexpected_risk`: checkpoint and return to planning; do not widen the task silently.
- After the convergence state reaches `ready_to_finish`, do not start another product-mutation or implement/check cycle; only complete final verification, spec capture, commit, and wrap-up.
Use the convergence decision/checkpoint as the source of truth when the check result and the workflow loop disagree.
